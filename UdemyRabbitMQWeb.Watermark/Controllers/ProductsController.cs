...
namespace UdemyRabbitMQWeb.Watermark.Controllers
{
    public class ProductsController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("Id,Name,Price,Stock,ImageName")] Product product, IFormFile ImageFile)
        {
            ...
            {
                var randomFileName = $"{Guid.NewGuid().ToString()}{System.IO.Path.GetExtension(ImageFile.FileName)}";

                var path = System.IO.Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/images", randomFileName);

                using (var stream = new FileStream(path, FileMode.Create))
                {
                    await ImageFile.CopyToAsync(stream);
                    _rabbitMQPublisher.Publish(new productImageCreatedEvent() { ImageName = randomFileName });
                }
